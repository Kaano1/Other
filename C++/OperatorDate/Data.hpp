...
class Data
{
	private:
		...
		friend ostream& operator <<(ostream& Obj, Data& oth);
	public:
		...
		int		get_Year() const;
		void	operator++(int);
		void	operator>(const Data& othDate);
};
...
ostream& operator <<(ostream& Obj, Data& oth)
{
	Obj << oth.day << " " << oth.monthString[oth.month - 1] << " " << oth.year << endl;
	return (Obj);
}

void	Data::operator>(const Data& oth)
{
	long	first;
	long	second;
